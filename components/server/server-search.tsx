...
export default function ServerSearch({ data }: ServerSearchProps) {
  ...
  /**
   * The onClick function navigates to different routes based on the type of entity (channel or member)
   * and the provided id.
   * @param  - The `onClick` function takes an object as its parameter, which has two properties:
   * @returns a call to the `router.push()` function. The specific route being pushed to depends on the
   * value of the `type` parameter. If `type` is `'member'`, the route will be
   * `/servers/${params?.serverId}/conversations/`. If `type` is `'channel'`, the route will be
